...
function renderizarCaixa(data) {
  ...
  const tbody = document.getElementById('caixa-tbody'); tbody.innerHTML = '';
  if (data.length === 0) {
...
function renderizarServicos(data) {
  ...
  data.forEach(s => {
    ...
    tbody.innerHTML += `<tr>
      ...
      <td>${s.tipo}</td><td>${s.descricao || '-'}</td><td>${formatMoney(val)}</td>
      <td>${formatMoney(estudio)}</td><td style="color:#34D399">${formatMoney(repasse)}</td>
      <td>${s.forma_pagamento}</td><td>${acoes}</td>
    </tr>`;
  });
...
function renderizarAgenda(data) {
  ...
  if (data.length === 0) {
    ...
    data.forEach(a => {
      ...
      tbody.innerHTML += `<tr>
        <td>${formatDateTime(a.data_hora)}</td><td>${a.cliente}</td><td>${a.tatuador_nome}</td>
        <td>${a.tipo_servico}</td><td>${formatMoney(a.valor_estimado)}</td>
...
function renderizarEstoquePiercing(piercings) {
  ...
  piercings.forEach(p => { if (p.quantidade > 0) opts += `<option value="${p.id}" data-preco="${p.preco_venda}">${p.nome} - ${formatMoney(p.preco_venda)} (Estoque: ${p.quantidade})</option>`; });
...
window.remarcarServico = async (servicoId) => {
  ...
  modal.innerHTML = `<div class="modal-content"><span class="close" onclick="fecharModal('${modalId}')">&times;</span><h2><i class="fas fa-calendar-alt"></i> Remarcar Serviço</h2>
  ...
  <button class="btn btn-primary" id="btn-confirmar-remarcacao">Confirmar Remarcação</button></div>`;
  document.body.appendChild(modal);
  document.getElementById('nova-data').value = servico.data.split('T')[0];
  document.getElementById('nova-hora').value = '';
  
  const btn = document.getElementById('btn-confirmar-remarcacao');
  btn.addEventListener('click', async () => {
    const novaData = document.getElementById('nova-data').value;
    const novaHora = document.getElementById('nova-hora').value;
    if (!novaData) { showAlert('Informe a nova data', 'error'); return; }
    let desc = `[Remarcado de ${formatDate(servico.data)}${novaHora ? ` às ${novaHora}` : ''}] ` + (servico.descricao || '');
    try {
      ...
      fecharModal(modalId); await carregarServicos(); atualizarDashboard();
...
async function criarServicoDoAgendamento(agendaId) {
  try {
    ...
    const novoServico = {
      ...
      tipo: agenda.tipo_servico, descricao: agenda.observacoes || `Agendamento confirmado em ${new Date().toLocaleDateString()}`,
...
window.excluirCaixa = async (id) => {
  if (confirm('Excluir?')) {
    try { await supabaseClient.from('caixa').delete().eq('id', id); await carregarCaixa(); atualizarDashboard(); showAlert('Excluído', 'success'); } catch (e) { showAlert('Erro ao excluir', 'error'); }
  }
};
window.filtrarCaixa = () => {
...
window.editarServico = async (id) => {
  ...
  if (item.finalizado) { showAlert('Serviço finalizado não pode ser editado.', 'warning'); return; }
  ...
  document.getElementById('servico-valor').value = item.valor_total; document.getElementById('servico-pagamento').value = item.forma_pagamento;
  document.getElementById('modal-servico').style.display = 'block'; calcularRepasse();
};
window.excluirServico = async (id) => {
  const servico = currentData.servicos.find(s => s.id === id);
  if (servico && servico.finalizado) { showAlert('Serviço finalizado não pode ser excluído.', 'warning'); return; }
  if (confirm('Excluir serviço?')) { try { await supabaseClient.from('servicos').delete().eq('id', id); await carregarServicos(); atualizarDashboard(); showAlert('Serviço excluído', 'success'); } catch (e) { showAlert('Erro ao excluir', 'error'); } }
};
window.filtrarServicos = () => {
  let f = [...currentData.servicos];
  if (document.getElementById('filtro-tatuador-servico').value) f = f.filter(s => s.tatuador_nome === document.getElementById('filtro-tatuador-servico').value);
  if (document.getElementById('filtro-tipo-servico').value) f = f.filter(s => s.tipo === document.getElementById('filtro-tipo-servico').value);
  if (document.getElementById('filtro-pagamento').value) f = f.filter(s => s.forma_pagamento === document.getElementById('filtro-pagamento').value);
  if (document.getElementById('filtro-data-servico').value) f = f.filter(s => s.data === document.getElementById('filtro-data-servico').value);
...
window.limparFiltrosServicos = () => {
  ['filtro-tatuador-servico', 'filtro-tipo-servico', 'filtro-pagamento', 'filtro-data-servico'].forEach(id => document.getElementById(id).value = '');
...
window.editarAgenda = async (id) => {
  const item = currentData.agenda.find(a => a.id === id); if (!item) return showAlert('Agendamento não encontrado', 'error');
  document.getElementById('agenda-id').value = item.id; const dt = new Date(item.data_hora);
  document.getElementById('agenda-data').value = dt.toISOString().split('T')[0]; document.getElementById('agenda-horario').value = dt.toTimeString().slice(0, 5);
  document.getElementById('agenda-cliente').value = item.cliente; document.getElementById('agenda-tatuador').value = item.tatuador_nome;
  document.getElementById('agenda-tipo').value = item.tipo_servico; document.getElementById('agenda-valor').value = item.valor_estimado;
  document.getElementById('agenda-status').value = item.status; document.getElementById('agenda-obs').value = item.observacoes || '';
...
window.filtrarAgenda = () => {
  let f = [...currentData.agenda];
  if (document.getElementById('filtro-tatuador-agenda').value) f = f.filter(a => a.tatuador_nome === document.getElementById('filtro-tatuador-agenda').value);
  if (document.getElementById('filtro-status-agenda').value) f = f.filter(a => a.status === document.getElementById('filtro-status-agenda').value);
  if (document.getElementById('filtro-data-agenda').value) f = f.filter(a => a.data_hora.split('T')[0] === document.getElementById('filtro-data-agenda').value);
  renderizarAgenda(f);
};
window.filtrarAgendaHoje = () => { document.getElementById('filtro-data-agenda').valueAsDate = new Date(); filtrarAgenda(); };
window.limparFiltrosAgenda = () => { ['filtro-tatuador-agenda', 'filtro-status-agenda'].forEach(id => document.getElementById(id).value = ''); document.getElementById('filtro-data-agenda').value = ''; renderizarAgenda(currentData.agenda); };
...
window.abrirModalPiercing = (id = null) => {
  ...
  if (id) { supabaseClient.from('piercings_estoque').select('').eq('id', id).single().then(({ data }) => { if (data) { document.getElementById('piercing-id').value = data.id; document.getElementById('piercing-nome').value = data.nome; document.getElementById('piercing-qtd').value = data.quantidade; document.getElementById('piercing-preco').value = data.preco_venda; document.getElementById('modal-piercing').style.display = 'block'; } }).catch(e => showAlert('Erro ao carregar piercing', 'error')); } 
...
window.salvarPiercing = async () => {
  ...
  try {
    ...
    fecharModal('modal-piercing'); await carregarPiercings(); await carregarVendasPiercing(); showAlert('Piercing salvo', 'success');
...
window.abrirModalMaterial = (id = null) => {
  ...
  if (id) { supabaseClient.from('materiais_estoque').select('').eq('id', id).single().then(({ data }) => { if (data) { document.getElementById('material-id').value = data.id; document.getElementById('material-nome').value = data.nome; document.getElementById('material-qtd').value = data.quantidade; document.getElementById('material-preco').value = data.valor_unitario; document.getElementById('modal-material').style.display = 'block'; } }).catch(e => showAlert('Erro ao carregar material', 'error')); } 
...
window.salvarMaterial = async () => {
  ...
  try {
    ...
    fecharModal('modal-material'); await carregarMateriais(); await carregarUsosMateriais(); showAlert('Material salvo', 'success');
...
window.importarBackup = async (input) => {
  ...
  try {
    ...
    if (!backup.servicos && !backup.agenda && !backup.caixa) throw new Error('Arquivo de backup inválido ou corrompido.');
    const info = backup.metadata ? `📅 Exportado em: ${new Date(backup.metadata.data_exportacao).toLocaleString('pt-BR')}\n📊 Registros: ${Object.values(backup.metadata.contagens || {}).reduce((a,b)=>a+b,0)}\n💰 Faturamento: ${formatMoney(backup.metadata.resumo_financeiro?.faturamento_total_servicos || 0)}` : '⚠️ Backup antigo sem metadados.';
    if (confirm(`Deseja importar este backup?\n\n${info}\n\n⚠️ Isso pode duplicar dados. Continuar?`)) {
      let inseridos = 0;
      const tabelas = ['servicos', 'agenda', 'caixa', 'piercings_estoque', 'vendas_piercing', 'materiais_estoque', 'usos_materiais'];
      for (const tabela of tabelas) {
        const dados = backup[tabela];
        if (Array.isArray(dados) && dados.length > 0) {
          for (const item of dados) {
            const { id, created_at, updated_at, ...dadosLimpos } = item;
            await supabaseClient.from(tabela).insert([dadosLimpos]); inseridos++;
          }
        }
      }
      showAlert(`Backup importado com sucesso! ${inseridos} registros inseridos. Recarregando...`, 'success');
...
window.sincronizarAgora = async () => { showAlert('Sincronizando dados...', 'info'); await Promise.all([carregarCaixa(), carregarServicos(), carregarAgenda(), carregarPiercings(), carregarVendasPiercing(), carregarMateriais(), carregarUsosMateriais()]); atualizarDashboard(); await carregarRelatorios(); showAlert('Sincronização concluída!', 'success'); };
...
function setupAuthTabs() {
  ...
  tabLogin.addEventListener('click', () => { tabLogin.classList.add('active'); tabRegister.classList.remove('active'); loginPanel.classList.add('active'); registerPanel.classList.remove('active'); });
  tabRegister.addEventListener('click', () => { tabRegister.classList.add('active'); tabLogin.classList.remove('active'); registerPanel.classList.add('active'); loginPanel.classList.remove('active'); });
}
