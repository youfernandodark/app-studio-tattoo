...
const LoadingUtils = {
  show: (message = 'Carregando...') => {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = `<div style="background:#1e1e2a;padding:20px 32px;border-radius:16px;color:white;"><i class="fas fa-spinner fa-pulse"></i> <span id="loading-message">Carregando...</span></div>`;
...
const Renderer = {
  ...
  renderPaginacao: (prefixo, total, paginaAtual, itensPorPagina, callback) => {
    ...
    let html = `<div class="paginacao">
      <button ${paginaAtual === 1 ? 'disabled' : ''} data-pagina="${paginaAtual - 1}">&laquo; Anterior</button>`;
  ...
  renderServicos(data) {
    ...
    const linhas = data.map(s => {
      ...
      return `<tr>
        <td>${DateUtils.formatDate(s.data)}</td><td>${escapeHtml(s.cliente)}</td>
  ...
  renderAgenda(data) {
    const linhas = data.map(a => {
      ...
      return `<tr>
        <td>${dataStr}</td><td>${horaStr}</td><td>${escapeHtml(a.cliente)}</td>
  ...
  renderEstoquePiercing(piercings) {
    ...
    tbody.innerHTML = piercings.length ? piercings.map(p => `<tr>
      <td>${escapeHtml(p.nome)}</td><td>${p.quantidade}</td><td>${MoneyUtils.format(p.preco_venda)}</td><td>${MoneyUtils.format(p.custo_unitario || 0)}</td>
      <td class="actions-cell">
  ...
  renderVendasPiercing(vendas) {
    ...
    if (tbody) tbody.innerHTML = vendas.length ? vendas.map(v => `<tr>
      <td>${DateUtils.formatDate(v.data)}</td><td>${escapeHtml(v.piercing?.nome || '?')}</td>
      <td>${v.quantidade}</td><td>${MoneyUtils.format(v.valor_total)}</td><td>${escapeHtml(v.cliente || '-')}</td>
...
const ServicosModule = {
  ...
  excluir: async (id) => {
    ...
    try { LoadingUtils.show('Excluindo...'); await DataService.deleteRecord('servicos', id); await DataService.loadServicos(AppState.paginacao.servicos.pagina); await atualizarDashboard(); AlertUtils.show('Serviço excluído', 'success'); } catch (e) { ErrorHandler.handle('excluir serviço', e); } finally { LoadingUtils.hide(); }
  },
...
const AgendaModule = {
  ...
  salvar: async () => {
    ...
    try {
      ...
      if (AgendaModule._isReagendamento) {
        ...
        if (dataOriginal !== dataNova) await gerarComprovanteAgendamentoPDF({ id, ...record, data_hora: dataHoraLocal, reagendado: true });
      } else if (!id) {
        ...
        if (novoAgendamento?.[0]?.id) await gerarComprovanteAgendamentoPDF({ id: novoAgendamento[0].id, ...record, data_hora: dataHoraLocal });
      }
  ...
  cancelar: async (id) => {
    ...
    try { LoadingUtils.show('Cancelando...'); await DataService.saveRecord('agenda', { status: 'Cancelado' }, id); await DataService.loadAgenda(AppState.paginacao.agenda.pagina); atualizarDashboard(); AlertUtils.show('Agendamento cancelado.', 'success'); } catch (e) { ErrorHandler.handle('cancelar agenda', e); } finally { LoadingUtils.hide(); }
  },
  excluir: async (id) => {
    if (!await ConfirmModal.show('Excluir este agendamento?')) return;
    try { LoadingUtils.show('Excluindo...'); await DataService.deleteRecord('agenda', id); await DataService.loadAgenda(AppState.paginacao.agenda.pagina); atualizarDashboard(); AlertUtils.show('Agendamento excluído', 'success'); } catch (e) { ErrorHandler.handle('excluir agenda', e); } finally { LoadingUtils.hide(); }
  },
...
async function gerarComprovanteAgendamentoPDF(dados) {
  ...
  try {
    ...
    const { data: studio, error } = await supabaseClient.from('studio_config').select('nome, endereco, instagram, whatsapp').eq('id', 1).single();
    if (error) throw new Error('Erro ao buscar dados do estúdio: ' + error.message);
    const nomeStudio = studio?.nome || 'DARK013TATTOO';
    const endereco = studio?.endereco || ''; const instagram = studio?.instagram || ''; const whatsapp = studio?.whatsapp || '';
    ...
    const dataFormatada = dataHora.toLocaleDateString('pt-BR'); const horaFormatada = dataHora.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    ...
    element.innerHTML = `
      ...
      <div style="border-top:1px solid #3A3A3A;padding:15px 0;">
        ...
        <p><strong>Tipo:</strong> ${escapeHtml(dados.tipo_servico)}</p>
  ...
  } catch (error) { console.error('Erro ao gerar PDF:', error); ErrorHandler.handle('gerar PDF', error); if (element && element.parentNode) document.body.removeChild(element); } finally { LoadingUtils.hide(); }
}
...
async function carregarAnalisePiercing() {
  try {
    ...
    DomUtils.setHtml('piercing-resumo', `
      <div style="background:#2a2a3a;border-radius:8px;padding:12px;flex:1;text-align:center;"><div style="color:#A0A0A0;">Vendas Totais</div><div style="font-size:1.5rem;font-weight:bold;">${MoneyUtils.format(totalVendas)}</div></div>
      <div style="background:#2a2a3a;border-radius:8px;padding:12px;flex:1;text-align:center;"><div style="color:#A0A0A0;">Custo Total</div><div style="font-size:1.5rem;font-weight:bold;color:#F87171;">${MoneyUtils.format(totalCusto)}</div></div>
      <div style="background:#2a2a3a;border-radius:8px;padding:12px;flex:1;text-align:center;"><div style="color:#A0A0A0;">Lucro</div><div style="font-size:1.5rem;font-weight:bold;color:#34D399;">${MoneyUtils.format(lucro)}</div><div style="font-size:0.8rem;color:#A0A0A0;">Margem ${margem}%</div></div>
      <div style="background:#2a2a3a;border-radius:8px;padding:12px;flex:1;text-align:center;"><div style="color:#A0A0A0;">Mais Vendido</div><div style="font-size:1.2rem;font-weight:bold;">${escapeHtml(maisVendido ? maisVendido[0] : '-')}</div><div style="font-size:0.9rem;">${maisVendido ? maisVendido[1] + ' un.' : ''}</div></div>`);
...
const PiercingModule = {
  ...
  salvar: async () => {
    ...
    const quantidade = parseInt(DomUtils.getValue('piercing-qtd')) || 0; const preco_venda = MoneyUtils.parse(DomUtils.getValue('piercing-preco')); const custo_unitario = MoneyUtils.parse(DomUtils.getValue('piercing-custo'));
    if (!nome) return AlertUtils.show('Nome obrigatório', 'error'); if (!id && (!custo_unitario || custo_unitario <= 0)) return AlertUtils.show('Para novo piercing, o Valor de Custo é obrigatório', 'error');
    try {
      LoadingUtils.show('Salvando...'); let quantidadeAnterior = 0; if (id) { const { data } = await supabaseClient.from('piercings_estoque').select('quantidade').eq('id', id).single(); quantidadeAnterior = data?.quantidade || 0; }
      ...
      DomUtils.setDisplay('modal-piercing', 'none'); await DataService.loadPiercings(); await DataService.loadVendasPiercing(); await carregarAnalisePiercing(); AlertUtils.show('Piercing salvo', 'success');
    } catch (e) { ErrorHandler.handle('salvar piercing', e); } finally { LoadingUtils.hide(); }
  },
  editar: (id) => PiercingModule.abrirModal(id),
  excluir: async (id) => { if (!await ConfirmModal.show('Excluir este piercing?')) return; try { await DataService.deleteRecord('piercings_estoque', id); await DataService.loadPiercings(); await DataService.loadVendasPiercing(); await carregarAnalisePiercing(); AlertUtils.show('Piercing excluído', 'success'); } catch (e) { ErrorHandler.handle('excluir piercing', e); } },
  registrarVenda: async () => {
    ...
    if (!piercingId) return AlertUtils.show('Selecione um piercing', 'error'); if (quantidade <= 0) return AlertUtils.show('Quantidade deve ser maior que zero', 'error');
    try {
      LoadingUtils.show('Registrando venda...'); const { data: piercing } = await supabaseClient.from('piercings_estoque').select('*').eq('id', piercingId).single();
...
const BackupModule = {
  exportar: async () => {
    try { LoadingUtils.show('Gerando backup...'); const tabelas = ['servicos', 'agenda', 'caixa', 'piercings_estoque', 'vendas_piercing', 'materiais_estoque', 'usos_materiais']; const backup = { data_exportacao: new Date().toISOString() }; for (const t of tabelas) { const { data } = await supabaseClient.from(t).select('*'); backup[t] = data || []; } const blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `backup-dark013-${DateUtils.nowDate()}.json`; a.click(); AlertUtils.show('Backup exportado', 'success'); } catch (e) { ErrorHandler.handle('exportar backup', e); } finally { LoadingUtils.hide(); }
  },
  importar: async (input) => {
    const file = input.files[0]; if (!file) return;
    try { LoadingUtils.show('Importando...'); const text = await file.text(); const backup = JSON.parse(text); if (!backup.servicos) throw new Error('Arquivo inválido'); if (!await ConfirmModal.show(`Importar backup de ${backup.data_exportacao}? Pode duplicar dados.`)) return; const tabelas = ['servicos', 'agenda', 'caixa', 'piercings_estoque', 'vendas_piercing', 'materiais_estoque', 'usos_materiais']; for (const t of tabelas) { const registros = backup[t] || []; for (const reg of registros) { const { id, ...rest } = reg; await supabaseClient.from(t).insert([rest]); } } AlertUtils.show('Backup importado com sucesso', 'success'); setTimeout(() => location.reload(), 1500); } catch (e) { ErrorHandler.handle('importar backup', e); } finally { LoadingUtils.hide(); input.value = ''; }
  }
};

const ExemplosModule = {
  popularPiercings: async () => { if (!await ConfirmModal.show('Adicionar piercings de exemplo?')) return; const exemplos = [{ nome: 'Piercing Nariz Cristal', quantidade: 10, preco_venda: 80.00, custo_unitario: 25.00 }, { nome: 'Piercing Septo Aço', quantidade: 8, preco_venda: 120.00, custo_unitario: 35.00 }, { nome: 'Piercing Lábio Argola', quantidade: 5, preco_venda: 70.00, custo_unitario: 20.00 }]; try { LoadingUtils.show('Adicionando...'); for (const item of exemplos) { const { data: existente } = await supabaseClient.from('piercings_estoque').select('id').eq('nome', item.nome).maybeSingle(); if (!existente) await supabaseClient.from('piercings_estoque').insert([item]); } await DataService.loadPiercings(); await DataService.loadVendasPiercing(); await carregarAnalisePiercing(); AlertUtils.show('Exemplos adicionados!', 'success'); } catch (e) { ErrorHandler.handle('exemplos piercings', e); } finally { LoadingUtils.hide(); } },
  popularMateriais: async () => { if (!await ConfirmModal.show('Adicionar materiais de exemplo?')) return; const exemplos = [{ nome: 'Agulha 1207RL', quantidade: 50, valor_unitario: 2.50 }, { nome: 'Tinta Preta Intenze', quantidade: 8, valor_unitario: 45.00 }, { nome: 'Luvas Descartáveis M', quantidade: 100, valor_unitario: 0.80 }]; try { LoadingUtils.show('Adicionando...'); for (const item of exemplos) { const { data: existente } = await supabaseClient.from('materiais_estoque').select('id').eq('nome', item.nome).maybeSingle(); if (!existente) await supabaseClient.from('materiais_estoque').insert([item]); } await DataService.loadMateriais(); await DataService.loadUsosMateriais(1, 10); AlertUtils.show('Exemplos adicionados!', 'success'); } catch (e) { ErrorHandler.handle('exemplos materiais', e); } finally { LoadingUtils.hide(); } }
};
...
const ConfigModule = {
  ...
  salvar: async () => { const novoValor = parseInt(DomUtils.getValue('config-repasse-estudio')); if (isNaN(novoValor) || novoValor < 0 || novoValor > 100) { AlertUtils.show('Porcentagem inválida (0-100).', 'error'); return; } try { LoadingUtils.show('Salvando configuração...'); const { error } = await supabaseClient.from('studio_config').update({ repasse_porcentagem_estudio: novoValor }).eq('id', 1); if (error) throw error; AppState.config.repasseEstudio = novoValor; DomUtils.setDisplay('modal-config-repasse', 'none'); AlertUtils.show('Porcentagem padrão atualizada!', 'success'); } catch (e) { ErrorHandler.handle('salvar config repasse', e); } finally { LoadingUtils.hide(); } }
};
...
async function testarConexao() {
  ...
  try { const { error } = await supabaseClient.from('caixa').select('id').limit(1); if (error) throw error; if (statusEl) { statusEl.innerHTML = '<i class="fas fa-check-circle"></i> Conectado ao Supabase'; statusEl.className = 'status-badge status-connected'; } return true; } catch (err) { if (statusEl) { statusEl.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Falha na conexão'; statusEl.className = 'status-badge status-error'; } ErrorHandler.handle('conexão', err); return false; }
}
...
async function carregarConfiguracoes() {
  try { const { data, error } = await supabaseClient.from('studio_config').select('repasse_porcentagem_estudio').eq('id', 1).single(); if (error) throw error; if (data && data.repasse_porcentagem_estudio != null) AppState.config.repasseEstudio = Number(data.repasse_porcentagem_estudio); } catch (e) { console.warn('Usando valor padrão de repasse:', e); }
}
