...
// IMPORTANTE: Configure Row Level Security (RLS) no Supabase para proteger seus dados.
// Esta chave é pública e deve ter políticas restritivas.
...
if (typeof supabase !== 'undefined') {
    ...
    alert('Erro: Supabase não disponível. Verifique sua conexão.');
}
...
function formatDate(d) {
    ...
    // Ajuste de timezone: adiciona um dia se necessário para evitar offset
...
function showAlert(msg, type) {
    ...
    alertDiv.innerHTML = `<i class="fas ${type === 'success' ? 'fa-check-circle' : type === 'error' ? 'fa-exclamation-circle' : 'fa-info-circle'}"></i> ${msg}`;
...
async function carregarCaixa() {
    try {
        ...
        showAlert('Erro ao carregar caixa: ' + e.message, 'error');
    }
}
...
async function carregarAgenda() {
    try {
        ...
        showAlert('Erro ao carregar agenda: ' + e.message, 'error');
    }
}
...
async function carregarPiercings() {
    try {
        ...
        showAlert('Erro ao carregar piercings: ' + e.message, 'error');
    }
}
...
async function carregarVendasPiercing() {
    try {
        ...
        showAlert('Erro ao carregar vendas: ' + e.message, 'error');
    }
}
...
async function carregarMateriais() {
    try {
        ...
        showAlert('Erro ao carregar materiais: ' + e.message, 'error');
    }
}
...
async function carregarUsosMateriais() {
    try {
        ...
        showAlert('Erro ao carregar usos: ' + e.message, 'error');
    }
}
...
function atualizarDashboard() {
    ...
    // Gráfico de faturamento mensal (últimos 6 meses considerando ano)
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        const soma = currentData.servicos
            .filter(s => {
                ...
                return dataServ.getMonth() === d.getMonth() && dataServ.getFullYear() === d.getFullYear();
            })
    ...
    chartTipos = new Chart(document.getElementById('chart-tipos').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                data: [tatuagens, piercingsServ],
                backgroundColor: ['#818CF8', '#C084FC']
            }]
        }
    });
}
...
async function carregarRelatorios() {
    ...
    currentData.servicos.forEach(s => {
        fat[s.tatuador_nome] = (fat[s.tatuador_nome] || 0) + (+s.valor_total || 0);
    });
    document.getElementById('faturamento-tatuador').innerHTML = Object.entries(fat)
        .map(([k, v]) => `<div><strong>${k}:</strong> ${formatMoney(v)}</div>`)
        .join('') || 'Sem dados';
...
// ==================== CAIXA: ADICIONAR ENTRADA AUTOMÁTICA (COM RECÁLCULO SIMPLES) ====================
// NOTA: Idealmente o recálculo de saldo deve ser feito via trigger no banco de dados.
// Este método mantém a lógica simples, mas pode ficar inconsistente se edições manuais ocorrerem.
...
// ==================== REMARCAR SERVIÇO (CORRIGIDO COM VALIDAÇÃO DE DATA FUTURA) ====================
...
window.salvarCaixa = async () => {
    ...
    try {
        let error;
        if (id) {
            error = (await supabaseClient.from('caixa').update(data).eq('id', id)).error;
        } else {
            error = (await supabaseClient.from('caixa').insert([data])).error;
        }
        if (error) throw error;
        ...
        // Aviso sobre necessidade de recálculo manual se editar registro antigo
        if (id) {
            showAlert('Atenção: Editar um lançamento pode desatualizar saldos posteriores. Recomenda-se usar triggers no banco.', 'info');
        }
    } catch (e) {
...
window.excluirCaixa = async (id) => {
    if (confirm('Excluir este lançamento? Isso pode afetar os saldos posteriores.')) {
        try {
            await supabaseClient.from('caixa').delete().eq('id', id);
            await carregarCaixa();
            atualizarDashboard();
            showAlert('Excluído (verifique os saldos manualmente)', 'success');
        } catch (e) {
...
window.abrirModalServico = () => {
    ...
    calcularRepasse();
    // Adiciona listener para recalcular ao digitar
    const valorInput = document.getElementById('servico-valor');
    const tatuadorSelect = document.getElementById('servico-tatuador');
    valorInput.removeEventListener('input', calcularRepasse);
    tatuadorSelect.removeEventListener('change', calcularRepasse);
    valorInput.addEventListener('input', calcularRepasse);
    tatuadorSelect.addEventListener('change', calcularRepasse);
};
...
window.salvarServico = async () => {
    ...
    try {
        let error;
        if (id) {
            error = (await supabaseClient.from('servicos').update(data).eq('id', id)).error;
        } else {
            error = (await supabaseClient.from('servicos').insert([data])).error;
        }
        if (error) throw error;
...
window.salvarAgenda = async () => {
    ...
    try {
        let error;
        if (id) {
            error = (await supabaseClient.from('agenda').update(data).eq('id', id)).error;
        } else {
            error = (await supabaseClient.from('agenda').insert([data])).error;
        }
        if (error) throw error;
...
window.filtrarAgendaHoje = () => {
    ...
    document.getElementById('filtro-data-agenda').value = hoje;
    filtrarAgenda();
};
...
window.salvarPiercing = async () => {
    ...
    const nome = document.getElementById('piercing-nome').value.trim();
    ...
    try {
        if (id) {
            await supabaseClient.from('piercings_estoque').update({ nome, quantidade, preco_venda }).eq('id', id);
        } else {
            await supabaseClient.from('piercings_estoque').insert([{ nome, quantidade, preco_venda }]);
        }
...
window.registrarVendaPiercing = async () => {
    ...
    try {
        const { data: piercing, error: fetchError } = await supabaseClient
...
window.salvarMaterial = async () => {
    ...
    try {
        if (id) {
            await supabaseClient.from('materiais_estoque').update({ nome, quantidade, valor_unitario }).eq('id', id);
        } else {
            await supabaseClient.from('materiais_estoque').insert([{ nome, quantidade, valor_unitario }]);
        }
...
window.registrarUsoMaterial = async () => {
    ...
    try {
        ...
        showAlert('Erro ao registrar uso: ' + e.message, 'error');
    }
};
    ...
    try {
        ...
        if (confirm(`Importar backup de ${backup.data_exportacao}? Isso pode duplicar dados.`)) {
            for (const s of backup.servicos || []) {
                const { id, ...rest } = s;
                await supabaseClient.from('servicos').insert([rest]);
            }
            for (const a of backup.agenda || []) {
                const { id, ...rest } = a;
                await supabaseClient.from('agenda').insert([rest]);
            }
            for (const c of backup.caixa || []) {
                const { id, ...rest } = c;
                await supabaseClient.from('caixa').insert([rest]);
            }
            for (const p of backup.piercings || []) {
                const { id, ...rest } = p;
                await supabaseClient.from('piercings_estoque').insert([rest]);
            }
            for (const v of backup.vendas || []) {
                const { id, ...rest } = v;
                await supabaseClient.from('vendas_piercing').insert([rest]);
            }
            for (const m of backup.materiais || []) {
                const { id, ...rest } = m;
                await supabaseClient.from('materiais_estoque').insert([rest]);
            }
            for (const u of backup.usos || []) {
                const { id, ...rest } = u;
                await supabaseClient.from('usos_materiais').insert([rest]);
            }
...
// ==================== EXEMPLOS (CORRIGIDO CASE-INSENSITIVE) ====================
