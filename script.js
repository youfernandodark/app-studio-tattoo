...
// ==================== FUNÇÕES DE AUTENTICAÇÃO (MANTIDAS ORIGINAIS) ====================
...
function renderizarServicos(data) {
  ...
  data.forEach(s => {
    ...
    // Fórmula de repasse: Thalia fica com 70%, estúdio com 30%. Outros: 100% repasse, 0% estúdio.
    ...
    tbody.innerHTML += `<tr><td>${formatDate(s.data)}</td><td>${s.cliente}</td><td>${s.tatuador_nome}</td><td>${s.tipo}</td><td>${s.descricao || '-'}</td><td>${formatMoney(val)}</td><td>${formatMoney(estudio)}</td><td style="color:#34D399">${formatMoney(repasse)}</td><td>${s.forma_pagamento}</td><td>${acoes}</td></tr>`;
  });
...
// ==================== 📊 RELATÓRIOS (FÓRMULAS APRIMORADAS) ====================
async function carregarRelatorios() {
  ...
  // Lucro Líquido = Parte do Estúdio (30% Thalia + 100% outros) - Despesas do Caixa
...
async function adicionarEntradaCaixa(data, valor, descricao) {
  try {
    ...
    const novoLancamento = { data: data, saldo_inicial: saldoInicial, entradas: valor, saidas: 0, saldo_final: saldoFinal, descricao: descricao };
...
window.remarcarServico = async (servicoId) => {
  ...
  modal.innerHTML = `<div class="modal-content"><span class="close" onclick="fecharModal('${modalId}')">&times;</span><h2><i class="fas fa-calendar-alt"></i> Remarcar Serviço</h2><div class="form-group"><label>Nova Data</label><input type="date" id="nova-data" required></div><div class="form-group"><label>Novo Horário (opcional)</label><input type="time" id="nova-hora"><small>Se informado, será registrado na descrição do serviço.</small></div><button class="btn btn-primary" id="btn-confirmar-remarcacao">Confirmar Remarcação</button></div>`;
...
// ==================== 💾 BACKUP (MELHORADO COM METADADOS) ====================
window.exportarBackup = async () => {
  try {
    ...
    // Cálculos para o resumo financeiro do backup
    ...
    const backup = {
      metadata: {
        ...
        exportado_por: currentUser?.email || 'Desconhecido',
        resumo_financeiro: {
          faturamento_total_servicos: totalFaturamento,
          saldo_atual_caixa: saldoAtual,
          moeda: 'BRL'
        },
        contagens: {
          servicos: servicos?.length || 0,
          agendamentos: agenda?.length || 0,
          lancamentos_caixa: caixa?.length || 0,
          piercings_estoque: piercings?.length || 0,
          vendas_piercing: vendas?.length || 0,
          materiais_estoque: materiais?.length || 0,
          usos_materiais: usos?.length || 0
        }
      },
      servicos: servicos || [],
      agenda: agenda || [],
      caixa: caixa || [],
      piercings_estoque: piercings || [],
      vendas_piercing: vendas || [],
      materiais_estoque: materiais || [],
      usos_materiais: usos || []
    };
...
window.importarBackup = async (input) => {
  ...
  try {
    ...
    const info = backup.metadata ? `
      📅 Exportado em: ${new Date(backup.metadata.data_exportacao).toLocaleString('pt-BR')}
      📊 Registros: ${Object.values(backup.metadata.contagens || {}).reduce((a,b)=>a+b,0)}
      💰 Faturamento: ${formatMoney(backup.metadata.resumo_financeiro?.faturamento_total_servicos || 0)}
    ` : '⚠️ Backup antigo sem metadados. Importação continuará normalmente.';
      ...
      for (const tabela of tabelas) {
        ...
        if (Array.isArray(dados) && dados.length > 0) {
          for (const item of dados) {
            const { id, created_at, updated_at, ...dadosLimpos } = item; // Remove IDs e timestamps automáticos
            await supabaseClient.from(tabela).insert([dadosLimpos]);
            inseridos++;
          }
        }
      }
